...
export function computeNudges(members: FamilyMember[], hidden: HiddenPeople = NO_HIDDEN_PEOPLE): Nudge[] {
  ...
  for (const m of members) {
    ...
    // Family Education and business CV qualifications share expiry behaviour,
    // while each nudge opens the record's own profile tab.
    for (const q of [...(m.cv?.qualifications || []).map(q => ({ ...q, destination: 'cv' })),
      ...(m.education?.qualifications || []).map(q => ({ ...q, destination: 'education' }))]) {
      if (!q.expiryDate) continue;
      const t = new Date(q.expiryDate).getTime();
      if (isNaN(t)) continue;
      const months = (t - now) / MONTH;
      const days = Math.round((t - now) / DAY);
      if (months < 0) out.push({ key: `${q.destination}qual-${m.id}-${q.id}`, memberId: m.id, icon: Award, tone: 'urgent', text: `${first}'s ${q.name} has expired`, tab: q.destination, date: q.expiryDate, days });
      else if (months <= 2) out.push({ key: `${q.destination}qual-${m.id}-${q.id}`, memberId: m.id, icon: Award, tone: 'warn', text: `${first}'s ${q.name} expires soon`, tab: q.destination, date: q.expiryDate, days });
    }
  }
