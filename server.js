...
- Use "list_add" to append a row to a list: household lists → vehicles (fields: name, make, model, year, registration, vin, fuelType, assignedMember, insurer, insuranceNumber, insuranceRenewal [YYYY-MM-DD], inspectionExpiry [YYYY-MM-DD, the §57a/Pickerl/MOT/TÜV due date], vignetteExpiry [YYYY-MM-DD], lastService [YYYY-MM-DD], serviceIntervalMonths [number], notes — capture whatever inspection/insurance/service dates the user gives so the app can remind them), pets (name, species, vet, vaccinations, microchip, notes), utilities (type, provider, accountNumber, notes — for electricity/gas/internet/phone ONLY, NOT addresses); finances lists → banks (bankName, accountHolder, iban, bic, notes), insurance (provider, type, policyNumber, renewalDate, notes), benefits (name, reference, notes); family timeline → list="timeline" (date, title, type, note); shopping list → list="shopping" (name). For shopping: each item gets its own {"kind":"list_add","list":"shopping","item":{"name":"<item name>"}} — one edit per item. All dates YYYY-MM-DD.
