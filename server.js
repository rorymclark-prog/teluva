...
- {"kind":"contact","name":<string>,"relation":<string>,"phone":<string>,"email":<string>}   // a shared family contact (school office, a friend, etc.) — NOT a doctor/dentist/specialist, use "provider" for those. Contact DETAILS only: for a birthday use "extended_birthday" below, never this kind. Both are fine for the same person — file the phone number here and the birthday there.
...
- {"kind":"extended_birthday","name":<string>,"relationship":<string or "">,"date":<MM-DD, no year>,"originalYear":<number, only if the user states or implies a birth year, else omit>,"notes":<string>}  // A BIRTHDAY FOR SOMEONE WHO IS NOT A FAMILY MEMBER — a grandparent, aunt, godparent, family friend, an ex-partner, a colleague. Use this for "Granny's birthday is March 3rd", "remember uncle Tom's birthday, 12 June", "my mum turns 70 on the 4th of May". NOT a one-off "calendar_event" (that fires once and never again) and NOT a "contact" (contact details only). This is the record the family's calendar, their exported calendar file and their phone notifications all read, so a birthday filed here recurs every year everywhere. "date" is MM-DD with NO year — if the user gives a full date like "3 March 1946", put "03-03" in date and 1946 in originalYear; if they don't say a year, OMIT originalYear entirely rather than guessing one. Someone who is a full family member on file gets their birthday via {"kind":"member","field":"birthdate"} instead — check FAMILY DATA's member list first. DEDUPE: FAMILY DATA carries "extendedBirthdays" already on file; if that person's birthday is already there, don't file it again — say so in your reply, and use "update_record" if a detail has changed.
__CV_EDIT_LINE__
...
- {"kind":"home_service","records":[{"date":<YYYY-MM-DD>,"work":<what was done / what had broken>,"by":<who did it — person or firm, or "">,"trade":<Plumber|Electrician|Boiler / heating|Locksmith|Handyman|Cleaner|Gardener|Appliance repair|Pest control|Other, or "">,"area":<what/where in the property — e.g. "Boiler","Kitchen","Roof" — or "">,"cost":<string or "">,"warrantyUntil":<YYYY-MM-DD the work is guaranteed to, or "">,"notes":<string or "">}]}  // WORK DONE ON THE PROPERTY — the house's equivalent of a vehicle service record. Use it whenever the user says a tradesperson has BEEN and DONE something ("the plumber came Tuesday and replaced the boiler valve", "we had the chimney swept in March", "Elektro Wagner rewired the kitchen, 400 euro, two-year guarantee"), and for scanned tradesman INVOICES, job sheets and work reports for the home. One object per visit/job ("work" is required; fill the rest only from what you were actually told or shown). There is only one property, so nothing needs matching. Put the tradesperson's name in "by" — if they are already in info.vendors the app links the two automatically, and if they are not, ALSO file a {"kind":"vendor"} edit in the same batch so the family can call them again. Record what happened, never a verdict: no "that is overdue", no next-service date you worked out yourself.
...
basic: name, nickname, birthdate, name_day, place_of_birth, nationality, languages, gender, spouse
  spouse is who an adult member is married to or partnered with, stored as a plain NAME ("Maria", "Maria Bauer"). NEVER INFER IT. Two adults appearing in the same family space, sharing a surname, sharing an address, or being the only two grown-ups on file are not evidence of a marriage — set this only when someone SAYS so ("my wife Maria", "Papa's partner is Elena", "she's married to Tom"). A relationship you assumed and filed is not correctable by the people it is wrong about, because they have no idea it is there. If the person named is also a family member on file, still write their name here; this field does not link records. When someone reports a separation, divorce or death, do not rewrite this field to describe it — clear it with {"kind":"clear_field","field":"spouse"} and, if the family wants the fact kept, put that in a timeline memory where it belongs.
...
- Use kind "passport" for passports, "contact" for people/places to phone (school, friend — NOT doctors, advisers or tradespeople), "provider" for any doctor/dentist/optician/specialist/pharmacy OR financial adviser/accountant/lawyer-notary/insurance broker/bank contact, "vendor" for a TRADESPERSON or household service, "number" for a loose reference number not tied to a person.
- Use "vendor" for the people a household calls when something breaks or needs doing: {"kind":"vendor","name":"<person or firm>","trade":"<Plumber|Electrician|Boiler / heating|Locksmith|Handyman|Cleaner|Gardener|Appliance repair|Pest control|Neighbour (spare key)|Other>","company":"<firm, if the name is a person>","phone":"...","afterHoursPhone":"<emergency/out-of-hours line, if given>","accountRef":"<customer or account number>","lastServiceDate":"<YYYY-MM-DD they last did work>","isUsual":true,"notes":"..."}. Set isUsual true only when the user says this is the one they normally use ("our plumber", "the guy we always call"). If the trade is not in the list, put the user's own word in "trade" anyway — it is preserved. A tradesperson is NOT a "contact" and NOT a "provider": provider is medical and financial professionals only, and a contact carries no trade, no account number and no out-of-hours line, which are exactly what is wanted at 11pm with water coming through the ceiling. Existing vendors are in FAMILY DATA under info.vendors — check there first and use "update_record" with targetKind "vendor" to correct or update one (e.g. a new last-service date) instead of filing a near-duplicate.
...
- ADDRESSES — pick the right target, NEVER use kind "number" or utilities for an address:
  • A SPECIFIC PERSON's address (where a family member lives — e.g. "Mia's address is...", "my address is...", or a Meldezettel/registration naming one person): store on THAT member with {"kind":"member","member":"<name>","field":"address","value":"<full street, city, postcode>"}. Family members can live at different addresses. Also field "phone" and "email" for a member's own contact details.
...
async function readFamilyOccasionSources(familyId) {
  const [membersSnap, ebSnap, annSnap, settingsSnap, refInfoSnap] = await Promise.all([
    ...
    // reference/info — FamilyInfo (numbers, CONTACTS, providers), NOT
    // info/info, which is the space's name and admin. See the header of
    // server/yearlyCelebrations.mjs.
    adminDb.doc(`families/${familyId}/reference/info`).get(),
  ]);
  const members = membersSnap.docs.map((d) => ({ id: d.id, ...(d.data() || {}) }));
  const stored = (ebSnap.exists ? ebSnap.data()?.extendedBirthdays : null) || [];
  // A legacy contact birthday counts as an extended birthday here, so it obeys
  // the same calendarDivisions toggle below and reaches subscribers the same
  // way. Normally empty — see contactsAsExtendedBirthdays.
  const extendedBirthdays = [
    ...stored,
    ...contactsAsExtendedBirthdays((refInfoSnap.exists ? refInfoSnap.data()?.contacts : null) || [], stored),
  ];
...
ABSOLUTE RULES:
- NEVER rename or Westernise the name to find a match. Do not turn Rory into Roderick, Rodrigo or Rodrigue; do not turn Ganga into Gangolf because the Namenskalender happens to hold one; do not Westernise Shyam into anything. "celebrationOf" must always be the person's OWN name, exactly as given — never a substitute you invented.
...
- MOVABLE (non-Gregorian, lunar/lunisolar/variable-date) celebrations: "dateType":"movable", name the RULE in "movableRule" (e.g. "Kartik Purnima", "Krishna Janmashtami") — NEVER invent one fixed Gregorian date for something that genuinely moves year to year. Then resolve it for the two specific years you are given, "currentYearDate" and "nextYearDate", both real Gregorian "YYYY-MM-DD" dates in those exact years. If you cannot resolve a year with confidence, OMIT that date field rather than guessing — a missing date is fixed by resolving it again later; a wrong one puts a false celebration on someone's calendar.
...
{"kind":"name_celebration","title":"Krishna Janmashtami — Shyam's Name Celebration","celebrationOf":"Shyam","matchType":"cultural","tradition":"Hindu / Vaishnava","explanation":"Shyam is one of the familiar names for Krishna, from the word for dark blue. Krishna Janmashtami celebrates Krishna's appearance and is the day most directly connected to the name.","source":"Hindu lunisolar calendar (Vaishnava almanac)","dateType":"movable","movableRule":"Krishna Janmashtami","religious":true}
Never "Shyam → Simon" or any other Western lookalike — there is no such connection, and inventing one is exactly what this feature must never do.

Example 2 — "Ganga" (no cultural background given), offered as TWO proposals, one recommended:
{"kind":"name_celebration","title":"Ganga Dussehra — Ganga's Name Celebration","celebrationOf":"Ganga","matchType":"cultural","tradition":"Hindu","explanation":"Ganga is the name of the river Ganges. Ganga Dussehra marks the day the river is said to have descended to earth, which is the origin story the name itself carries.","source":"Hindu lunisolar calendar (Jyeshtha Shukla Dashami)","dateType":"movable","movableRule":"Ganga Dussehra","religious":true,"recommended":true}
{"kind":"name_celebration","title":"Ganga Saptami — Ganga's Name Celebration","celebrationOf":"Ganga","matchType":"cultural","tradition":"Hindu","explanation":"Ganga Saptami marks the river's reappearance from the sage Jahnu — a second day held for the same river, and a genuine alternative.","source":"Hindu lunisolar calendar (Vaishakha Shukla Saptami)","dateType":"movable","movableRule":"Ganga Saptami","religious":true,"recommended":false}
Ganga Dussehra is recommended because the descent of the river is the origin story the name itself carries — but both are genuine, so both are offered and the family chooses.
